...
import React from 'react';
import { Link, NavLink, useNavigate } from 'react-router-dom';
import { X } from 'lucide-react'; // X icon close korar jonno
import loginIcon from '../assets/loginIcon.svg';
...
const Sidebar = ({ onLogout, isOpen, setIsOpen }) => {
  ...
  const menuItems = [
    ...
    { name: 'Services', path: '/services', iconPath: "M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" },
  ...
  return (
    <>
      {/* 1. Mobile Overlay: Sidebar khola thakle background dark hobe */}
      {isOpen && (
        <div 
      ...
      <div className={`
        fixed top-0 left-0 z-50 h-[92%] w-64 bg-white border rounded-2xl border-purple-400 font-nunito p-5 shadow-lg flex flex-col transition-transform duration-300 ease-in-out
      ...
      `}>
        ...
        {/* Logo Section */}
        <div className="flex justify-center mb-10 pt-4">
          <img src={loginIcon} alt="Logo" className="w-20 h-20 object-contain" />
        </div>
        ...
        <nav className="flex-1 space-y-4 overflow-y-auto">
          {menuItems.map((item) => (
            <NavLink
              key={item.name}
              to={item.path}
              onClick={() => setIsOpen(false)} // Mobile-e click korle auto close hobe
              className={({ isActive }) => `
                w-full flex items-center p-4 rounded-2xl transition-all duration-300 font-bold
                ${isActive 
                  ? 'bg-[#EFE6FD] text-purple-700 border-2 border-purple-500 shadow-[0px_4px_0px_0px_rgba(168,85,247,1)]' 
